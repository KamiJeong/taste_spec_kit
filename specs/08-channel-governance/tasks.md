...
- [x] CH-T001 채널/멤버/가입요청/정렬 테이블 스키마 설계
- [x] CH-T002 Drizzle migration 추가 및 롤백 검증
- [x] CH-T003 테이블 제약조건/인덱스(중복 pending 요청 방지 포함) 반영
- [x] CH-T004 API request/response zod schema 정의
...
- [x] CH-T010 강퇴 서비스 구현(owner/manager 규칙 반영)
- [x] CH-T011 탈퇴 서비스 구현(member quit, owner 탈퇴 제한)
- [x] CH-T012 소유권 이전 서비스 구현(owner -> target member)

## Phase 3: API Wiring (P1)

- [x] CH-T013 channel controller 엔드포인트 추가
- [x] CH-T014 인증/세션/CSRF 정책 정합성 적용
- [x] CH-T015 에러 코드/응답 envelope 정합성 맞춤
- [x] CH-T016 Swagger/OpenAPI 문서화 반영

## Phase 4: Ordering (P2)

- [x] CH-T017 owner 채널 관리 순서 변경 API 구현(DnD 저장)
- [x] CH-T018 사용자 개인 채널 리스트 순서 변경 API 구현(DnD 저장)
- [x] CH-T019 reorder 입력 유효성/원자성 보장 처리

## Phase 5: Limits, Audit, Reliability (P2)

- [x] CH-T020 가입 채널 수 상한 200 강제 및 설정화 구현
- [x] CH-T021 승인/거절/강퇴/권한변경/소유권이전 감사 로그 연계
- [x] CH-T022 join request/approval rate limit 정책 반영
- [x] CH-T023 동시성 충돌 시나리오 처리(트랜잭션/조건부 업데이트)
