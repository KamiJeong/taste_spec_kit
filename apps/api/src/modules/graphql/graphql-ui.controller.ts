...
<html lang="en">
  ...
  <body>
    <div class="wrap">
      ...
      <p>Endpoint: <code>/graphql</code> | Supports <code>sid</code> cookie auth and <code>Authorization: Bearer</code> auth.</p>
      <div class="panel">
        ...
        <div id="sessionStatus" class="small">Session: unknown</div>
      </div>
      <div class="grid">
        <div>
          <label>Query</label>
  ...
  createChannel(name: $name) { id name role }
      ...
      async function refreshSession() {
        ...
        const hasSid = !!cookieOf("sid");
        const hasCsrf = !!cookieOf("csrfToken");
        sessionStatus.textContent = "Session: " + (hasSid ? "sid=present" : "sid=missing") +
      ...
      function useQuery() { q.value = queryTpl; v.value = JSON.stringify(queryVars, null, 2); }
      function useMutation() { q.value = mutationTpl; v.value = JSON.stringify(mutationVars, null, 2); }
      function useMyChannels() { q.value = myChannelsTpl; v.value = JSON.stringify({}, null, 2); }
      function useCreateChannel() { q.value = createChannelTpl; v.value = JSON.stringify(createChannelVars, null, 2); }
      async function run() {
        let variables;
        ...
        if (csrf.value.trim()) headers["x-csrf-token"] = csrf.value.trim();
