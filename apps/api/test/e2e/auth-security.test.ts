...
async function run() {
  ...
  try {
    const email = `e2e-security-${suffix}@example.com`;
    const signup = await requestJson(baseUrl, "/api/v1/auth/signup", {
      method: "POST",
      body: { email, password: "Passw0rd!", name: "E2E Security" }
    });
    ...
    for (let i = 0; i < 12; i += 1) {
      const attempt = await requestJson(baseUrl, "/api/v1/auth/login", {
        method: "POST",
        body: { email: `e2e-ratelimit-${suffix}@example.com`, password: "WrongPassw0rd!" }
      });
    ...
    console.log("e2e: security csrf and rate-limit ok");
  } finally {
    await app.close();
  }
}
