...
수정된 원칙:
- 기술 스택 참조 규칙 강화: 모든 speckit 명령은 `specs/00-tech-stack.md`를 명시적으로 참조해야 함
- SSOT, Pinned-Stack 규칙 보강 (명시적 명령 참조 요구 추가)
  
추가된 섹션:
- 명시적 명령 참조 규칙 (speckit 명령의 SSOT 참조 요구)
  
제거된 섹션: 없음
  
템플릿 업데이트 상태:
- ✅ .specify/templates/plan-template.md (헌법 확인 문구 보강)
- ✅ .specify/templates/spec-template.md (스택 참조 요구 문구 추가)
- ✅ .specify/templates/tasks-template.md (스택 참조 요구 문구 추가)
- ✅ .specify/templates/commands/plan.md (새로 생성됨 - 명시적 참조 예시 포함)
- ✅ .specify/templates/commands/tasks.md (새로 생성됨 - 명시적 참조 예시 포함)
- ✅ specs/00-tech-stack.md (헤더에 명령 참조 안내 문구 추가)
...
- 모든 `/speckit` 명령은 실행 시 **명시적으로** `specs/00-tech-stack.md`를 참조해야 합니다
- 환경 변수, 설정값은 단일 소스에서 관리하고 다른 곳에서 참조만 합니다
- 중복이 발견되면 즉시 통합하고 단일 소스로 리팩토링해야 합니다

**근거**: 중복된 정보는 불일치를 초래하고, 유지보수 비용을 증가시키며, 버그의 원인이 됩니다. SSOT는 일관성을 보장하고변경의 영향 범위를 명확히 합니다。
...
- 모든 `/speckit` 명령(예: `/speckit.plan`, `/speckit.tasks`)은 실행 시 **명시적으로** `specs/00-tech-stack.md`를 참조해야 합니다.

**근거**: `speckit` 명령이 기술 스택을 참조하도록 의무화하면 명령이 생성하는 계획/작업/명세 산출물이 프로젝트의 SSOT와 일치하게 되어 중복/비일관성 위험을 낮춥니다.
...
- [ ] 모든 `/speckit` 명령 출력에 `specs/00-tech-stack.md` 참조가 포함되었는가? (**명시적 스택 참조 확인**)
...
**버전**: 1.1.0 | **제정일**: 2026-02-09 | **최종 수정일**: 2026-02-10
